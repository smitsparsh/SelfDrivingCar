...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    S = np.linalg.inv(h_jac @ p_cov_check @ h_jac.T + R)
    K_k = p_cov_check @ h_jac.T @ S
    # 3.2 Compute error state
    delX = K_k @ (y_k - p_check)
    # 3.3 Correct predicted state
    p_hat = p_check + delX[0:3]
    v_hat = v_check + delX[3:6]
    q_hat = Quaternion(axis_angle=delX[6:9]).quat_mult_right(q_check, out='np')
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    Q_km = np.identity(6)
    Q_km[:, :3] *= delta_t ** 2 * var_imu_f
    Q_km[:, -3:] *= delta_t ** 2 * var_imu_w
    ...
    q_check = Quaternion(axis_angle=w * delta_t).quat_mult_right(q_est[k - 1])

    F_k = np.identity(9)
    F_k[0:3, 3:6] = delta_t * np.identity(3)
    F_k[3:6, 6:9] = -C_ns @ (skew_symmetric(f.reshape(3, 1)))

    # 2. Propagate uncertainty
    p_cov_check = F_k @ p_cov_check @ F_k.T + l_jac @ Q_km @ l_jac.T

    # 3. Check availability of GNSS and LIDAR measurements
    if gnss_i < gnss.t.shape[0] and gnss.t[gnss_i] == imu_f.t[k - 1]:
        y_k = gnss.data[gnss_i]
        p_check, v_check, q_check, p_cov_check = measurement_update(var_gnss, p_cov_check, y_k, p_check, v_check,
                                                                    q_check)
        gnss_i += 1

    if lidar_i < lidar.t.shape[0] and lidar.t[lidar_i] == imu_f.t[k - 1]:
        y_k = lidar.data[lidar_i]
        p_check, v_check, q_check, p_cov_check = measurement_update(var_lidar, p_cov_check, y_k, p_check, v_check,
                                                                    q_check)
    # Update states (save)
    p_est[k] = p_check
    v_est[k] = v_check
