...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    if gnss_i < 49:
        ...
        if isGNSSAvailable and ~isLIDARAvilable:
            ...
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov_check, y_k, p_check, v_check,
                                                                        q_check)
            gnss_i = gnss_i + 1
        elif ~isGNSSAvailable and isLIDARAvilable:
            y_k = lidar.data[lidar_i]
            p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_lidar, p_cov_check, y_k, p_check, v_check,
                                                                        q_check)
            lidar_i = lidar_i + 1
        elif isGNSSAvailable and isLIDARAvilable:
            y_k = gnss.data[gnss_i]
